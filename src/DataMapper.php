...
class DataMapper implements IteratorAggregate
{
     ...
     */
    public function __construct($id = null, $configFromFile = null)
    {
        ...
        if (isset(DataMapper::$common[DMZ_CLASSNAMES_KEY][$this_class])) {
            ...
            DataMapper::$common[DMZ_CLASSNAMES_KEY][$this_class] = $common_key = \elanpl\DM\helpers\inflector_helper::singular($this_class);
        }
        ...
        if ($is_dmz) {
            ...
            DataMapper::$config = $configFromFile;
        ...
        if (!isset(DataMapper::$common[$common_key])) {
            ...
            //TODO: czy te języki są potrzebne ?
            ...
            // 	if(file_exists(DataMapper::$config['path'].'language/'.$idiom.'/'.$lang_file.'_lang'.EXT))
            // 	{
            ...
            if (!empty(DataMapper::$config['production_cache'])) {
                ...
                if (!is_dir($cache_folder = DataMapper::$config['production_cache'])) {
                    ...
                    $cache_folder = DataMapper::$config['patch'].DataMapper::$config['production_cache'];
                }
            ...
            if (!$loaded_from_cache) {
                ...
                if (empty($this->table)) {
                    $this->table = \elanpl\DM\helpers\inflector_helper::plural(get_class($this));
                }
    ...
    {
        // foreach (array_merge(array(_L2_PATH), self::$model_paths) as $path)
        // {
        // 	// Prepare file
     ...
     * @param bool   $local   If TRUE, only loads the extension into this object
     */
    ...
    public function __get($name)
    {
        ...
        if ($name == 'db') {
            $this->db = \elanpl\DM\DB\Connection::getInstance(DataMapper::$config['db_config_function']());
            $this->db->setDatabase($this->dbName);
            //$this->db->queries = null; //& $CI->db->queries;
            //$this->db->query_times = null; //& $CI->db->query_times;
        ...
        if ($has_many || $has_one) {
            ...
            $class = DataMapper::$config['model_namespace'].$related_properties['class'];
            $this->{$name} = new $class();
            ...
            if ($this->exists() &&
                    ($has_many && ($this->auto_populate_has_many || $this->has_many[$name]['auto_populate']))
                                         || ($has_one && ($this->auto_populate_has_one || $this->has_one[$name]['auto_populate']))) {
                $this->{$name}->get();
            }
        ...
        $name_single = \elanpl\DM\helpers\inflector_helper::singular($name);
        if ($name_single !== $name) {
    ...
    public function __call($method, $arguments)
    {
        ...
        //AG: added for Twig: {{ site.site_type.name }} .site_type. is function
        if (array_key_exists($method, $this->has_one) ||
                     array_key_exists($method, $this->has_many)) {
            return  $this->$method;
        }
     ...
     * @param int|null $offset offset the results when limiting
     *
    ...
    public function get_iterated($limit = null, $offset = null)
    {
        ...
        $object->db->resetQuery();
        // Clear the query related list from the clone
        $object->_query_related = array();

        // Build iterator
        $this->_dm_dataset_iterator = new DMDatasetIterator($object, $this->get_raw($limit, $offset, true));
    ...
    public function get_paged($page = 1, $page_size = 50, $page_num_by_rows = false, $info_object = 'paged', $iterated = false)
    {
        ...
        $count_query->db->resetSelect();
        $count_query->db->resetOrderBy();
        $total = $count_query->db->isDistinct() ? $count_query->count_distinct() : $count_query->count();
    ...
    public function update($field, $value = null, $escape_values = true)
    {
        ...
        if (empty($field)) {
            \elanpl\DM\helpers\inflector_helper::show_error('Nothing was provided to update.');
        }
...
//                $query = $this->db->query($sql);
    ...
    protected function _handle_related()
    {
        if (!empty($this->parent)) {
            ...
            if ($has_many || $has_one) {
                ...
                \elanpl\DM\helpers\inflector_helper::show_error("DataMapper Error: '".$this->parent['model']."' is not a valid parent relationship for $this_model.  Are your relationships configured correctly?");
            }
        }
    ...
    protected function _subquery($query, $args)
    {
        ...
        if ($query == 'select') {
            ...
            $alias = $args[1];
            // we can't use the normal select method, because CI likes to breaky
            $this->db->selectRaw("$sql AS $alias");
    ...
    public function group_start($not = '', $type = 'AND ')
    {
        ...
        //$prefix = ($this->db->isEmptyWhere() ) ? '' : $type;
        //$value =  $prefix . $not . str_repeat(' ', $this->_group_count) . ' (';
        //$this->db->whereRaw($value);
        if ($type == 'OR ') {
            $this->db->whereOr();
        }
        if ($not) {
            $this->db->whereRaw($not);
        }

        for ($i = 0; $i < $this->_group_count; ++$i) {
            $this->db->whereStartGroup();
        }
    ...
    protected function _where_in($key = null, $values = null, $not = false, $type = 'AND ')
    {
        ...
        if ($not) {
            $this->db->where_not_int($this->add_table_name($key), $values);
        } else {
    ...
    protected function _where_between($key = null, $value1 = null, $value2 = null, $not = false, $type = 'AND ')
    {
        ...
        $this->db->whereRaw($this->add_table_name($key).' '.($not ? 'NOT ' : '')
                        .'BETWEEN '.$this->db->escape($value1).' AND '.$this->db->escape($value2));
    ...
    protected function _handle_default_order_by()
    {
        ...
        if ($this->db->isEmptyOrderBy() &&
            (
                $this->db->isEmptySelect() ||
                $this->db->hasSelect('*') ||
                                $this->db->hasSelect($sel_protect) ||
                                $this->db->hasSelect($sel)
            )) {
    ...
    public function _get_related_properties(&$related_field, $try_singular = false)
    {
        ...
        if (strpos($related_field, '/') !== false) {
            ...
            if (isset($this->has_many[$related_field])) {
                ...
                if ($try_singular) {
                    $rf = \elanpl\DM\helpers\inflector_helper::singular($related_field);
                    $ret = $this->_get_related_properties($rf);
                    if (is_null($ret)) {
                        \elanpl\DM\helpers\inflector_helper::show_error("Unable to relate {$this->model} with $related_field.");
                    } else {
                        $related_field = $rf;
    ...
    public function _add_related_table($object, $related_field = '', $id_only = false, $db = null, &$query_related = null, $name_prepend = '', $this_table = null)
    {
        ...
        if ($relationship_table == $this->table && in_array($other_column, $this->fields)) {
            ...
            } elseif (!in_array($object_as, $query_related)) {
                $db->join($object->table.' '.$object_as, $object_as.'.id = '.$this_table.'.'.$other_column, 'LEFT OUTER');
                $query_related[] = $object_as;
            }
        }
        // the extra in_array column check is for has_one self references
        elseif ($relationship_table == $object->table && in_array($this_column, $object->fields)) {
            // has_one relationship without a join table
                ...
                if ($query->num_rows() == 0) {
                    ...
                    if (isset($this->has_many[$related_field])) {
                        ...
                        if (isset($object->has_one[$other_field])) {
                            ...
                            return true;
                        } elseif (isset($object->has_many[$other_field])) {
    ...
    public function _get_relationship_table($object, $related_field = '')
    {
        ...
        if (!empty($related_properties['join_table'])) {
            ...
            if ($this->table == $table) {
                ...
                $p_this_model = \elanpl\DM\helpers\inflector_helper::plural($this_model);
                $p_other_model = \elanpl\DM\helpers\inflector_helper::plural($other_model);
    ...
    protected function _count_related($related_field, $object = '')
    {
        ...
        $class = DataMapper::$config['model_namespace'].$rel_properties['class'];
    ...
    public function set_join_field($related_field, $field, $value = null, $object = null)
    {
        ...
        if (is_array($related_field)) {
            ...
            return;
        } elseif (is_object($related_field)) {
            $object = $related_field;
            $related_field = $object->model;
    ...
    {
        if (!empty($this->{$field})) {
            $query = $this->db->where(array($field => $this->{$field}))->get($this->table, 1, 0);
    ...
    protected function _unique_pair($field, $other_field = '')
    {
        if (!empty($this->{$field}) && !empty($this->{$other_field})) {
            $query = $this->db->where(array($field => $this->{$field},
    ...
    public function production_cache()
    {
        ...
        if (!empty(DataMapper::$config['production_cache'])) {
            ...
            if (!is_dir($cache_folder = DataMapper::$config['production_cache'])) {
                // if not, it's relative to the application path
                $cache_folder = DataMapper::$config['path'].DataMapper::$config['production_cache'];
            }
            if (file_exists($cache_folder) && is_dir($cache_folder) && is_writeable($cache_folder)) {
                $common_key = DataMapper::$common[DMZ_CLASSNAMES_KEY][get_class($this)];
                $cache_file = $cache_folder.'/'.$common_key.EXT;
                ...
                if (!$fp = @fopen($cache_file, 'w')) {
                    \elanpl\DM\helpers\inflector_helper::show_error('Error creating production cache file: '.$cache_file);
                }
    ...
    protected function _relationship($type = '', $definition = array(), $name = 0)
    {
        ...
        if (isset($definition['reciprocal'])) {
            ...
            $definition['reciprocal'] = ($definition['reciprocal'] && $type == 'has_many' && $definition['class'] == get_class($this));
        } else {
    ...
    protected function _run_get_rules()
    {
        ...
        foreach ($this->_field_tracking['get_rules'] as $field) {
            ...
            foreach ($rules as $rule => $param) {
                ...
                } elseif ($this->_extension_method_exists('rule_'.$rule)) {
                    // Run an extension-based rule.
                    $result = $this->{'rule_'.$rule}($field, $param);
                } elseif (method_exists($this->form_validation, $rule)) {
                    // Run rule from CI Form Validation
                    $result = $this->form_validation->{$rule}($this->{$field}, $param);
                } elseif (function_exists($rule)) {
                    // Run rule from PHP
 ...
 */
spl_autoload_register('elanpl\DM\DataMapper::autoload');
